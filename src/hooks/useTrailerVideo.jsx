...
export const useTrailerVideo = (id) => {
  const dispatch = useDispatch()
  const trailer = useSelector(store => store.movie.trailerVideos[id]) // per movie ID
  ...
  useEffect(() => {
    const gettingMovieTrailer = async () => {
      ...
      const trailerVideo = filterTrailer.length ? filterTrailer[0] : json?.results[0]
      dispatch(addTrailerVideo({ movieId: id, trailer: trailerVideo }))
    }

    if (!trailer) gettingMovieTrailer()
  }, [id, trailer, dispatch])
}
